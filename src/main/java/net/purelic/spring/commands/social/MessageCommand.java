...
public class MessageCommand implements ProxyCommand {
    ...
    private static void sendFancyPM(ProxiedPlayer sender, ProxiedPlayer recipient, String message, boolean isSender) {
        ...
        pm.addExtra(isSender ? getMessageName(recipient, sender) : getMessageName(sender, recipient));
        pm.addExtra(ChatColor.GRAY + "): " + ChatColor.RESET + message);
        (isSender ? sender : recipient).sendMessage(pm);
    }

    @SuppressWarnings("deprecation")
    private static TextComponent getMessageName(ProxiedPlayer player, ProxiedPlayer viewer) {
        TextComponent component = new TextComponent(ChatColor.DARK_AQUA + NickUtils.getDisplayName(player, viewer));
